...
In this R Markdown file, we will provide a template for the DARE Deluxe Data
Challenge. The main steps covered here will be:
...
# Data.
source(
  here::here("pipeline_R/functions_R", "R_data_functions.R"),
  local = knitr::knit_global()
)
# Helper.
source(
  here::here("pipeline_R/functions_R", "R_helper_functions.R"),
  local = knitr::knit_global()
)
# Plotting.
source(
  here::here("pipeline_R/functions_R", "R_plotting_functions.R"),
  local = knitr::knit_global()
)
# Preprocessing.
source(
  here::here("pipeline_R/functions_R", "R_preprocessing.R"),
  local = knitr::knit_global()
)
...
y_var <- "v2_WE.16.03.B3"   # Name of the gauge to make predictions for.
...
cat(
  "Number of features: ", ncol(train_val_x),
...
cat("\nPredict variable: ", paste0('"', colnames(train_val_y), '"  '), sep = "")
...
We can either split this randomly if we are just modelling as a plain
regression problem (set `shuffle = True`), or we can split it sequentially if
we are modelling as a time series problem (set `shuffle = False`).

```{r chunk_split}

# randomly or sequentially split the data into training and validation sets
# there is no benefit to using a validation in our simple linear model
val_split <- TRUE
shuffle <- FALSE
...
if (val_split == TRUE) {
  ...
  if (!outcome_predictor_count_match) {
    cat(
      "\nWarning: ",
      "mismatched number of outcome and predictor rows in training set.\n",
      sep = ""
    )
  }
  ...
  train_y <- filter(
    train_val_y, row_number() %in% seq(1, (nrow(train_val_x) - val_absolute))
  )
  val_y <- filter(
    train_val_y, row_number() %in% seq(
      (1 + nrow(train_val_x) - val_absolute), nrow(train_val_x)
    )
  )
...
We will use the very simplistic approach of just assessing the correlation to
the target variable and selecting the most useful features.

```{r chunk_predictors}

# select the top n features - use NULL for all features
...
if (!is.null(best_n)) {
  ...
  if (val_split == TRUE) {
    corr_mtrx <- head(
      abs(psych::corr.test(x = bind_cols(train_x_all, train_y))$r), -1
    )
  } else {
    corr_mtrx <- head(
      abs(psych::corr.test(x = bind_cols(train_x_all, train_val_y))$r), -1
    )
  }

  # print the best n predictors
  corr_mtrx <- head(
    corr_mtrx[order(corr_mtrx[, ncol(corr_mtrx)], decreasing = TRUE), ],
    best_n
  )
  for (i in seq_len(nrow(corr_mtrx))) {
    cat(
      corr_mtrx[i, ncol(corr_mtrx)], "  ", row.names(corr_mtrx)[i], "\n",
      sep = ""
    )
  }
...
if (use_scaler) {
  ...
  test_x <- scale(
    test_x, center = attr(train_x, "scaled:center"),
    scale = attr(train_x, "scaled:scale")
  )
  if (val_split) {
    val_x <- scale(
      val_x, center = attr(train_x, "scaled:center"),
      scale = attr(train_x, "scaled:scale")
    )
  }
  ...
  train_y[, 1] <- DescTools::StripAttr(
    datawizard::rescale(train_y, to = c(0, 1))
  )
  test_y[, 1] <- DescTools::StripAttr(
    datawizard::rescale(test_y[, ], to = c(0, 1), range = scaler_y_range)
  )
  if (val_split) {
    val_y[, 1] <- DescTools::StripAttr(
      datawizard::rescale(val_y, to = c(0, 1), range = scaler_y_range)
    )
  }
...
### Train a model
We will use a simple linear regression model as a baseline.
You can implement your own model here.
...
model <- lm(
  formula = as.formula(paste0(y_var, " ~ .")), data = cbind(train_x, train_y)
)
...
## predict on data
train_y_pred <- data.frame(
  tmp = predict(model, newdata = as.data.frame(train_x))
)
names(train_y_pred) <- y_var
#
test_y_pred <- data.frame(
  tmp = predict(model, newdata = as.data.frame(test_x))
)
